...
thirty_countries_covid %>% 
  drop_na() %>% 
  dplyr::select(location, gdp_per_capita, total_deaths_per_million,total_vaccinations,new_deaths) %>% 
  group_by(location, gdp_per_capita, total_deaths_per_million) %>% 
  ggplot(aes(new_deaths,total_vaccinations))+
  geom_point(na.rm = TRUE) +
  geom_smooth( se = FALSE) +
  ylab("New Death by COVID") +
  xlab("Total Vaccinations")+
  labs(title =" Case recorded from 2020-12-13 to 2021-11-01")
```
